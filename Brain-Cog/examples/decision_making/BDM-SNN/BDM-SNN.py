...
def chooseAct(Net, input, weight_trace_d1, weight_trace_d2):
    ...
    raise RuntimeError("BDM-SNN produced no PM action spike within 500 internal steps")
...
if __name__ == "__main__":
    ...
    num_state = 9
    num_action = 2
    seed = int(os.environ.get("BDMSNN_SEED", "0"))
    random.seed(seed)
    np.random.seed(seed)
    ...
    device = torch.device(os.environ.get(
        "BDMSNN_DEVICE", "cuda" if torch.cuda.is_available() else "cpu"))
    communication_rank = int(os.environ.get("BDMSNN_RRR_RANK", "0"))
    communication_window = int(os.environ.get("BDMSNN_RRR_WINDOW", "128"))
    communication_warmup = int(os.environ.get("BDMSNN_RRR_WARMUP", "0"))
    communication_refit_interval = int(os.environ.get("BDMSNN_RRR_REFIT_INTERVAL", "1"))
    communication_mode = os.environ.get("BDMSNN_RRR_MODE", "striatum_to_output")
    communication_links_value = os.environ.get("BDMSNN_RRR_LINKS")
    communication_links = (tuple(name for name in communication_links_value.split(",") if name)
                           if communication_links_value else None)
    communication_lossless_only = os.environ.get("BDMSNN_RRR_LOSSLESS_ONLY", "0") == "1"
    communication_metrics_path = os.environ.get("BDMSNN_COMMUNICATION_METRICS_PATH")
    frame_rate = int(os.environ.get("BDMSNN_FRAME_RATE", "60"))
    print("BDM-SNN device:", device, "seed:", seed)
    DM = BDMSNN(
        num_state, num_action, weight_exc, weight_inh, "lif",
        communication_rank=communication_rank,
        communication_window=communication_window,
        communication_warmup=communication_warmup,
        communication_refit_interval=communication_refit_interval,
        communication_mode=communication_mode,
        communication_links=communication_links,
        communication_lossless_only=communication_lossless_only).to(device)
    ...
    weight_trace_d2 = torch.zeros(con_matrix1.shape, dtype=torch.float,
                                  device=device)
    telemetry_path = os.environ.get("BDMSNN_TELEMETRY_PATH")
    telemetry = None
    if telemetry_path:
        telemetry = {key: [] for key in (
            "frame", "state", "action", "reward", "source_dlpfc",
            "direct_current", "indirect_current", "strd1_spike",
            "strd2_spike", "weight_d1", "weight_d2")}
    ...
    state = lastState
    num = 0
    max_frames = int(os.environ.get("BDMSNN_MAX_FRAMES", "30000"))
    num_reward = []
    num_score = []
    while not collide:
        num = num + 1
        if num > max_frames:
            break
        input = torch.zeros((num_state), dtype=torch.float, device=device)
        if frame_rate > 0:
        ...
        if frame_clock % 2 == 0 or frame_clock == 1:
            ...
            if telemetry is not None:
                decision_source = input.detach().cpu().numpy().copy()
                decision_direct = (input @ DM.connection[0].weight).detach().cpu().numpy().copy()
                decision_indirect = (input @ DM.connection[1].weight).detach().cpu().numpy().copy()
                decision_strd1 = DM.out[0].detach().cpu().numpy().copy()
                decision_strd2 = DM.out[1].detach().cpu().numpy().copy()
        ...
        if frame_clock % 2 == 0 or frame_clock == 1 or collide:
            ...
            if frame_clock > 0 and not collide:
                ...
                if telemetry is not None:
                    telemetry["frame"].append(frame_clock)
                    telemetry["state"].append(state)
                    telemetry["action"].append(int(action.item()))
                    telemetry["reward"].append(reward)
                    telemetry["source_dlpfc"].append(decision_source)
                    telemetry["direct_current"].append(decision_direct)
                    telemetry["indirect_current"].append(decision_indirect)
                    telemetry["strd1_spike"].append(decision_strd1)
                    telemetry["strd2_spike"].append(decision_strd2)
                    telemetry["weight_d1"].append(DM.connection[0].weight.detach().cpu().numpy().copy())
                    telemetry["weight_d2"].append(DM.connection[1].weight.detach().cpu().numpy().copy())
    ...
    print("run_summary:", "frames=", num, "rewards=", len(num_reward_np),
          "score=", score, "reward_sum=", float(num_reward_np.sum()))
    if communication_rank or communication_metrics_path:
        communication_metrics = DM.communication_metrics()
        print("communication_summary:", communication_metrics)
        if communication_metrics_path:
            with open(communication_metrics_path, "w", encoding="utf-8") as output:
                json.dump({
                    "frames": num,
                    "reward_updates": len(num_reward_np),
                    "score": score,
                    "reward_sum": float(num_reward_np.sum()),
                    "communication": communication_metrics,
                    "refit_history": DM.communication_history(),
                }, output, indent=2)
                output.write("\n")
            print("communication_metrics_saved:", communication_metrics_path)
    np.save('lif_reward_l.npy', num_reward_np)
    np.save('lif_score_l.npy', num_score_np)
    if telemetry is not None:
        np.savez_compressed(telemetry_path, **{
            key: np.asarray(values) for key, values in telemetry.items()
        })
        print("telemetry_saved:", telemetry_path)
    print(score)
